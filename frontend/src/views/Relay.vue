...
const relaySecret = computed(
  () => settingsStore.settings?.openai_relay_secret || "",
);
const relayURL = computed(() =>
  relayStore.status?.running
    ? relayStore.status.url || `http://127.0.0.1:${relayPort.value}`
...
onMounted(() => {
  void relayStore.ensureStatusLoaded();
  void mitmStore.ensureStatusLoaded();
});

const relayBooting = computed(
  () =>
    !settingsStore.hasLoadedOnce ||
...
<template>
  <PageLoadingSkeleton v-if="relayBooting" variant="relay" class="w-full" />

  <SkeletonOverlay v-else :active="relayLoading" label="Relay 状态刷新中">
              ...
              <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    ...
                    "
                  >
                    {{ relayStore.status?.running ? "运行中" : "未启动" }}
                  </span>
                </div>
                <p
                  class="mt-1 text-[12px] leading-relaxed text-ios-textSecondary dark:text-ios-textSecondaryDark"
                >
                  本地 OpenAI 兼容 API 端点，复用 MITM 号池 JWT
                  自动轮换。支持流式 SSE，可对接
                  ChatGPT-Next-Web、LobeChat、OpenAI SDK 等任意客户端。
                </p>
              </div>
            </div>
        ...
        <div class="space-y-5 p-6">
          ...
          >
            <div class="min-w-0">
              ...
              >
                ...
                />
                {{
                  relayStore.status?.running ? "Relay 运行中" : "Relay 未启动"
                }}
              </div>
              <p
                class="mt-1 text-[12px] leading-relaxed text-ios-textSecondary dark:text-ios-textSecondaryDark"
              >
                {{
                  relayStore.status?.running
                    ? `监听 127.0.0.1:${relayStore.status?.port || relayPort}`
                    : "启动后号池中的 API Key 将自动用于对话请求"
                }}
              </p>
            </div>
            <IToggle
              :modelValue="!!relayStore.status?.running"
          ...
          >
            <div class="flex items-start gap-3">
              ...
              />
              <div
                class="text-[12px] leading-relaxed text-amber-700 dark:text-amber-300"
              >
                号池为空，请先在「号池」页面通过
                <strong>API Key 导入</strong> 添加
                <code class="rounded bg-black/5 px-1 dark:bg-white/10"
                  >sk-ws-01-...</code
                >
          ...
          <!-- Endpoint 信息 -->
          <div v-if="relayStore.status?.running" class="space-y-3">
            <div
            ...
            >
              ...
              <div class="space-y-2.5">
                ...
                <div class="flex items-center gap-2">
                  <div
                    class="flex-1 min-w-0 rounded-[14px] border border-black/[0.05] bg-black/[0.02] px-3 py-2.5 dark:border-white/[0.06] dark:bg-white/[0.03]"
                  >
                    <div
                      class="text-[10px] font-bold uppercase tracking-[0.15em] text-ios-textSecondary dark:text-ios-textSecondaryDark"
                    >
                      Base URL
                    </div>
                    <div
                      class="mt-0.5 truncate font-mono text-[12px] font-semibold text-ios-text dark:text-ios-textDark select-all"
                    >
                      {{ relayURL }}/v1
                    </div>
                  </div>
                ...
                <div class="flex items-center gap-2">
                  <div
                    class="flex-1 min-w-0 rounded-[14px] border border-emerald-500/15 bg-emerald-500/[0.04] px-3 py-2.5"
                  >
                    <div
                      class="text-[10px] font-bold uppercase tracking-[0.15em] text-ios-textSecondary dark:text-ios-textSecondaryDark"
                    >
                ...
                <div v-if="relaySecret" class="flex items-center gap-2">
                  ...
                  >
                    ...
                    <div
                      class="mt-0.5 font-mono text-[12px] text-ios-text dark:text-ios-textDark select-all"
                    >
                      {{ relaySecret }}
                    </div>
                  </div>
                ...
                <div
                  v-else
                  class="rounded-[14px] border border-black/[0.05] bg-black/[0.02] px-3 py-2.5 dark:border-white/[0.06] dark:bg-white/[0.03]"
                >
                  <div
                    class="text-[10px] font-bold uppercase tracking-[0.15em] text-ios-textSecondary dark:text-ios-textSecondaryDark"
                  >
                  ...
                  <div
                    class="mt-0.5 text-[12px] text-ios-textSecondary dark:text-ios-textSecondaryDark"
                  >
                    未设置（无需鉴权）· 可在「设置」中配置
                  </div>
                </div>
          ...
          <!-- 未运行说明 -->
          <div
            v-if="!relayStore.status?.running"
            class="rounded-[22px] border border-black/[0.05] bg-white/70 p-4 shadow-sm dark:border-white/[0.06] dark:bg-white/[0.04]"
          >
            <div class="flex items-center gap-2 mb-3">
              <Shield
            ...
            <div
              class="space-y-2.5 text-[12px] leading-relaxed text-ios-textSecondary dark:text-ios-textSecondaryDark"
            >
              <div class="flex gap-2.5">
                <span
                  class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-violet-500/10 text-[10px] font-bold text-violet-600 dark:text-violet-300"
                  >1</span
                >
                <span
                  >在「号池」页面通过
                  <strong class="text-ios-text dark:text-ios-textDark"
                    >API Key 导入</strong
                  >
                  添加
                  <code class="rounded bg-black/5 px-1 dark:bg-white/10"
              ...
              <div class="flex gap-2.5">
                ...
                >
                <span
                  >打开上方开关启动 Relay（端口和密钥可在「设置」中修改）</span
                >
              </div>
              <div class="flex gap-2.5">
                <span
                ...
                <span
                  >将 Base URL
                  <code class="rounded bg-black/5 px-1 dark:bg-white/10"
                    >http://127.0.0.1:{{ relayPort }}/v1</code
                  >
                  填入你的 OpenAI 客户端</span
                >
              </div>
              <div class="flex gap-2.5">
                <span
    ...
    <template #skeleton>
      <PageLoadingSkeleton variant="relay" class="w-full" />
    </template>
  </SkeletonOverlay>
</template>
